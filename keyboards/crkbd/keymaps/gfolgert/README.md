...
The number layer is activated by holding or double-tapping "D+F." Double-tapping enters into the layer without the need for holding, and Escape returns to base layer. On the right side, the numbers are **ordered chronologically, top-down** with 0 on the spacebar. For keyboard shortcuts that include a number, **this layer is also activated with "W+D" so modifiers are more comfortably within reach**.
